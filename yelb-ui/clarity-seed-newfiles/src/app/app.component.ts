...
gradient: boolean;
view: any[] = [700, 200];
ngOnInit(){this.getvotes(); this.getstats(); this.getwebserver();}
...
getwebserver(): void {
    const url = `${this.appserver}/api/getwebserver`;
    console.log("connecting to app server " + url);
    this.http.get(url)
                .map((res: Response) => res.json())
                .subscribe(res => {console.log(res, res.webserver, res.webserver_name); this.servers = res})
    }

...
configure :production do
  set :redishost, "redis-server.yelb-db.svc.cluster.local"
  set :port, 4567
  set :yelbdbhost => "yelb-db.yelb-db.svc.cluster.local"
  set :yelbdbport => 5432
end
configure :test do
  set :redishost, "redis-server.yelb-db.svc.cluster.local"
...
get '/api/getwebserver' do
    headers 'Access-Control-Allow-Origin' => '*'
    headers 'Access-Control-Allow-Headers' => 'Authorization,Accepts,Content-Type,X-CSRF-Token,X-Requested-With'
    headers 'Access-Control-Allow-Methods' => 'GET,POST,PUT,DELETE,OPTIONS'

        content_type 'application/json'
    @webserver = request.env['REMOTE_ADDR']
    @webserver_name = request.env['HTTP_HOSTNAME']
    @servers = '{"webserver": "' + @webserver + '"' + ", " +  '"webserver_name": "' + @webserver_name + '"' + "}"

end #get /api/webserver
